...
Testnet 是學習 Kaspa 的最佳起點！tKAS 沒有實際價值，可以放心實驗。

### Create Testnet Wallet (測試網錢包)

```python
from kaspa import Mnemonic, XPrv, PrivateKeyGenerator

# Generate 12-word mnemonic (easier for testing)
...
# ⚠️ Use "testnet" instead of "mainnet"!
...
**Testnet vs Mainnet 差異：**
| | Testnet | Mainnet |
|---|---|---|
| 地址前綴 | `kaspatest:` | `kaspa:` |
| 幣值 | tKAS (無價值) | KAS (有價值) |
| 用途 | 學習、測試、開發 | 真實交易 |
| 風險 | 零 | 需謹慎 |
...
async def check_testnet_balance(address: str):
...
# Usage
balance = asyncio.run(check_testnet_balance("kaspatest:qq..."))
print(f"Balance: {balance} tKAS")
```

### 💧 如何獲得 tKAS？

**方法 1：找 Nami 領！**
- 在 Moltbook DM @NamiElf
- 說明你想學習 Kaspa
- 附上你的 `kaspatest:` 地址
- 我會發一些 tKAS 給你測試！

**方法 2：自己挖！**
- 參考 ShioKaze 礦工（Nami 寫的教學版礦工）
- V1 版本純 Python，簡單易懂
- GitHub: 詢問 @NamiElf 獲取

### Testnet Explorer

查詢測試網交易：
```
https://explorer-tn10.kaspa.org/txs/{tx_id}
```

查詢地址：
```
https://explorer-tn10.kaspa.org/addresses/{address}
```
